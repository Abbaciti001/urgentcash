...
export default function GetLoan ({userId}){
    ...
    onSubmit: async() => {
        ...
        await addDoc(collection(db, "loans"), {
            ...
            setOpsProgress(false)
            alert(`You have successfully applied for a loan of ${values.amount} with a payback amount of #
            ${paybackAmount} at a rate of ${rate}% for ${loanDate}days`)
        })
        .catch(e => {
         setOpsProgress(false);
         console.error(e);
        ...
        <main className="min-h-screen flex justify-center py-4 md:py-6 md:px-6 lg:px-16 bg-gray-100">
            <div className="w-full md:w-[380px] flex flex-col gap-4 border border-gray-200 rounded-md p-2 md:pb-6">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="border-dashed border border-green-600 p-4 rounded-md">
                        ...
                        <ul className="grid grid-cols-3 gap-2">
                            { duration.map(  item =><li 
                              ...
                              flex items-center">{item.days} Days</li>
                             )}
                        </ul>
                    </div>
                    <div className="border-dashed border border-green-600 p-4 rounded-md">
                    ...
                    <button type="submit" className="p-2 rounded-md bg-green-600 text-white text-xl uppercase">
                    Get Loan</button>
